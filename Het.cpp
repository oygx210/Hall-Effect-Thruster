...
int main()
{
	...
	for (int n = 0; n < n_nodes; n++)
	{
		...
		else if (volume.nodes[n].type == NEUMANN1) solver.g[n] = 0; /*phi_wall*/
		else if (volume.nodes[n].type == NEUMANN2) solver.g[n] = 0; /*phi_wall*/
	...
	for (ts = 0; ts < 200; ts++)
	{
		...
		cout <<ts<<";"<<ions.particles.size()<<";"<< ions2.particles.size() << ";" << ions3.particles.size() << ";" <<
			max_den << ";" << max_den2 << ";" << max_den3 << ";" << MASS << ";" << MOMENTUM << ";" << ENERGY << ";" << lambda_d << endl;
...
void InjectIons(Species *species, Volume& volume, FESolver& solver, double dt)
{
	...
	double num_per_sec = species->den0 * ION_VELOCITY * area; /*N'oublier pas de modifier PLASMA_DENS pour chaque espèce*/
		...
		for (int c = 0; c < n_elements; c++)
		{
			...
			for (int g = 0; g < ng; g++)
			{
				...
				for (int i = 0; i < 3; i++) {
					...
					c[0] = 4.07e-10;
					c[1] = 0.77;
					c[2] = 2 * Kb * 273.15 / mr;	//Bird's reference params at 316.0922 K
